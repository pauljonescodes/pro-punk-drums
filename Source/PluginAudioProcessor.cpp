...
juce::AudioProcessorValueTreeState::ParameterLayout PluginAudioProcessor::createParameterLayout() {
    ...
    for (const auto& pair : PluginUtils::getUniqueMidiNoteMicCombinations()) {
        ...
        for (const std::string& micId : micIds) {
            ...
            layout.add(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID { gainParameterId, 2 }, gainName, constants::gainNormalizableRange, 0.0f));
            ...
            layout.add(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID { panParameterId, 2 }, panName, constants::panNormalizableRange, 0.0f));
            ...
            layout.add(std::make_unique<juce::AudioParameterBool>(juce::ParameterID { phaseParameterId, 2 }, phaseName, false));
        }
    }
