...
juce::AudioProcessorValueTreeState::ParameterLayout PluginAudioProcessor::createParameterLayout()
{
	...
	layout.add(std::make_unique<juce::AudioParameterBool>(
		juce::ParameterID{ AudioParameters::changedComponentId, 1 },
		stringToTitleCase(AudioParameters::changedComponentId),
		false));
...
void PluginAudioProcessor::parameterChanged(const juce::String &parameterId, float newValue)
{
	...
	const auto changed = mAudioProcessorValueTreeStatePtr->getParameter(AudioParameters::changedComponentId);
	changed->setValue(1.0f);
