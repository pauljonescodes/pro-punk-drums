...
#endif
{
	...
	mSynthesiserPtr = std::make_unique<PanningSamplerSynthesiser>();

	int totalNumSamples = samples::bitRate * samples::maxRecordingLengthInSeconds * 100;

	mInternalBufferPtr = std::make_unique<juce::AudioBuffer<float>>(1, totalNumSamples);
	...
	for (const auto& instrument : samples::instrumentVector) {
		...
		case samples::InstrumentEnum::electricSnare:
		{
			...
			mSynthesiserPtr.get()->addSample(resourceName,
				samples::bitRate,
				samples::bitDepth,
				midiNote,
		...
		case samples::InstrumentEnum::sideStick:
		{
			for (int intensityIndex = 0; intensityIndex < samples::intensityIdVector.size(); intensityIndex++) {
				...
				for (int variationIndex = 0; variationIndex < samples::standardVariationsCount; variationIndex++) {
					...
					mSynthesiserPtr->addSample(resourceName,
						samples::bitRate,
						samples::bitDepth,
						midiNote,
						generalmidi::midiNoteToStopNotesMap.at(midiNote),
...
void ABKit2AudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
	...
	if (mInternalBufferPtr != nullptr && mInternalBufferPtr->getNumChannels() > 0) {
		const float* monoChannelData = mInternalBufferPtr->getReadPointer(0);

		for (int sample = 0; sample < mInternalBufferPtr->getNumSamples(); ++sample) {
			float monoSample = monoChannelData[sample];

			// Copy/mix mono signal into both left and right channels of the stereo output buffer
			for (int channel = 0; channel < totalNumOutputChannels; ++channel) {
				buffer.addSample(channel, sample, monoSample);
			}
		}
	}
}
